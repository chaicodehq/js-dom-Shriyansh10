...
export function applyBaseStyle(element, color, size) {
  ...
  element.style.cssText = `background-color:${color};width:${size}px;height:${size}px;border-radius:50%;`;
  return element;
}
...
export function toggleVisibility(element) {
  // Your code here
  if(!element) return null;
  if(element.style.display === 'none') {element.style.display = ''; return '';}
  else element.style.display = 'none'; return 'none';
}
